...
contract ERC20SubscriptonTest is Test, PermitSignature, TokenProvider, GasSnapshot {
    ...
    function getTransferDetails(address to, uint256 amount)
        private
        pure
        returns (IERC20Subscription.SignatureTransferDetails memory)
    {
        return IERC20Subscription.SignatureTransferDetails({to: to, requestedAmount: amount});
    }

    // teststhat funds are correctly transferred from the owner to the recipient upon payment
    function test_CollectPayment() public {
        uint256 salt = 0;
        uint256 timeInterval = 0;
        IERC20Subscription.PermitTransferFrom memory permit =
            defaultERC20SubscriptionPermit(address(token0), salt, timeInterval);
        bytes memory sig = getPermitERC20SubscriptionSignature(permit, fromPrivateKey, DOMAIN_SEPARATOR);

        IERC20Subscription.Subscription memory subscription = IERC20Subscription.Subscription({
            owner: from,
            signature: sig,
            permit: permit,
            transferDetails: getTransferDetails(address2, defaultAmount)
        });
